{ lib, lib', config, pkgs, ... }: let
  ...
  inherit (lib.types) enum int attrsOf submodule path;
  ...
  themes = pkgs.runCommandLocal "matugen-themes" {} ''
    ...
    ${getExe cfg.package} image ${cfg.image} \
      ...
      --config <(cat <<EOF
      [templates]
      ${attrsToLines (name: template: "${name} = { input_path = '${template.input}', output_path = '$out/${baseNameOf template.input}' }") cfg.templates}
      EOF)
  '';
in {
  options = {
        ...
        type = attrsOf (submodule ({ config, ... }: {
          options = {
            ...
            output = mkOption {
              type = path;
              readOnly = true;
              default = "${themes}/${baseNameOf config.input}";
